...
-- 8. DOCUMENT_REQUIREMENTS: Define what documents are needed for each case type
CREATE TABLE IF NOT EXISTS public.document_requirements (
  ...
  category TEXT, -- e.g., "Identification", "Finance", "Civil"
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);
...
VALUES ('law-pilot-bucket', 'law-pilot-bucket', false)
...
TO authenticated
WITH CHECK (bucket_id = 'law-pilot-bucket');

CREATE POLICY "Allow public upload to guests folder"
ON storage.objects FOR INSERT
TO public
WITH CHECK (bucket_id = 'law-pilot-bucket' AND name LIKE 'guests/%');

CREATE POLICY "Users can view their own documents"
ON storage.objects FOR SELECT
TO authenticated
USING (
  bucket_id = 'law-pilot-bucket' AND 
  (
    -- For documents associated with a case
    EXISTS (
      SELECT 1 FROM public.documents d
      JOIN public.cases c ON d.case_id = c.id
      WHERE d.file_path = storage.objects.name AND c.client_id = auth.uid()
    )
    OR
    -- For guest documents belonging to the user's session (pre-login)
    (storage.objects.name LIKE 'guests/%')
    OR
    -- Lawyers/Admins can see everything
    EXISTS (
      SELECT 1 FROM public.profiles p
      WHERE p.id = auth.uid() AND p.role IN ('lawyer', 'admin')
    )
  )
);
...
-- 12. SEED DATA (Optional, for document requirements)
INSERT INTO public.document_requirements (case_type, name, category, is_required)
VALUES 
('Green Card for spouse located in the U.S.', 'Passport (Biographical Page)', 'Identification', true),
('Green Card for spouse located in the U.S.', 'Birth Certificate', 'Civil Document', true),
('Green Card for spouse located in the U.S.', 'Marriage Certificate', 'Civil Document', true),
('Fiancé(e) Visa', 'Passport (Biographical Page)', 'Identification', true),
('Fiancé(e) Visa', 'Evidence of Intent to Marry', 'Supporting', true),
('H-1B Work Visa', 'Resume / CV', 'Professional', true),
('H-1B Work Visa', 'Educational Degree', 'Professional', true)
